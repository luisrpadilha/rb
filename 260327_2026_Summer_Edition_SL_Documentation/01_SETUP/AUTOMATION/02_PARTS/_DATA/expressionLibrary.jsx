{
    "nameCheck": function(dataLayer, part) {
        // Returns 100 if the input layer name in the "DATA" comp and input layerName.split("_")[part] match
        const dataText = thisLayer.comp("DATA").layer(dataLayer).text.sourceText.toLowerCase();
    ...
    "variantCheck": function() { //used in all global comps + EDITIONS MAIN
        const variantSource = thisLayer.comp("DATA").layer("VARIANT").text.sourceText.toLowerCase();
    ...
    "productNameCheck": function() { //Checks string after first Underscore
        const nameSource = thisLayer.comp("DATA").layer("PRODUCT NAME").text.sourceText.toLowerCase();
    ...
    "countryCheck": function() { // only for Variants text bottom, also checks label Type
        ...
        const labelSource = thisLayer.comp("DATA").layer("LABEL TYPE").text.sourceText.toLowerCase();

        var countrySource = thisLayer.comp("DATA").layer("COUNTRY").text.sourceText.toLowerCase();
    ...
    "countryVariantCheck": function() { //used for Brazil & Turkey because their Bottom Text is also Variant dependend
        ...
        var countrySource = thisLayer.comp("DATA").layer("COUNTRY").text.sourceText.toLowerCase();
        var variantSource = thisLayer.comp("DATA").layer("VARIANT").text.sourceText.toLowerCase();
    ...
    "countryOnlyCheck": function() { // checks only country (for rhombus text bottom, for banderole for example)
        var countrySource = thisLayer.comp("DATA").layer("COUNTRY").text.sourceText.toLowerCase();
        var layerCountry = thisLayer.name.split("_")[0].toLowerCase();
    ...
    "audioCheck": function() {
        const soundCheck = thisLayer.comp("DATA").layer("AUDIO ON/OFF").text.sourceText.toLowerCase();
    ...
    "labelColor": function() { //returns hex color for the colored can label based on the variant and layer color
        try {
            if (thisLayer.transform.opacity !== 0) {
                ...
                const sku = thisLayer.comp("DATA").layer("SKU").text.sourceText.toUpperCase();
    ...
    "rbFont": function(textData, textType) { // Applies the correct alphabet font for each character in the text
        ...
        // Checks if Flag is SF Colro or selection of Default red or FCO Color was made in Data Comp  
    "flagColor": function(){

        const variantSource = thisLayer.comp("DATA").layer("VARIANT").text.sourceText.toLowerCase();
        const sfCheck = variantSource.split("-")[0];
        const flagColor=thisLayer.comp("SHOT04_CLEAN").layer("Control").effect("Default_Color RED")("Checkbox");
        const defaultColor = thisLayer.comp("DATA").layer("DISRUPTOR DEFAULT COLOR").text.sourceText.toLowerCase();
        // If Data Comp Variant is not SF and default Flag Color checkbox in shot4_clean comp is enabled  (=deafult)
        if(sfCheck!="sf" && defaultColor == "yes" ){
            return 0;
        }
         // If Data Comp Variant is not SF and default Flag Color checkbox in shot4_clean comp is disabled   (=FCO_COLOR)
        else if (sfCheck != "sf" && defaultColor== "no" ){
            return 1;
        }
        // If Edition Can is SF Overules all
        else {
            return 2
